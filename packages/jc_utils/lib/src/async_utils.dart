...
class StreamControllerReEmitOnce<T> {
  T? latestValue;

  final StreamController<T> _controller = StreamController<T>.broadcast();
  bool get hasListener => _controller.hasListener;

  StreamControllerReEmitOnce({T? initialValue}) : latestValue = initialValue;

  Stream<T> get stream {
    if (latestValue != null) {
      var newStream = _controller.stream.newStreamWithInitialValue(latestValue as T);
      latestValue = null as T?;
      return newStream;
    } else {
      return _controller.stream;
    }
  ...
  void add(T newValue) {
    if(!_controller.hasListener) latestValue = newValue;
    _controller.add(newValue);
  }
  ...
  void listen(Function(T) onData, {Function? onError, void Function()? onDone, bool? cancelOnError}) {
    if(latestValue != null) onData(latestValue as T);
    _controller.stream.listen(onData, onError: onError, onDone: onDone, cancelOnError: cancelOnError);
  }
...
class AsyncUtils{
  ...
  static Future<void> wait(List<Future Function()> futures) async {
    await Future.wait(futures.map((e) => Future(() => e())));
  }
}