...
import 'package:jc_widgets/src/map_location_picker/animated_pin.dart';
import 'package:jc_widgets/src/map_location_picker/custom_info_window.dart';
import 'package:jc_widgets/src/map_location_picker/models/map_controller.dart';
import 'dart:ui' as ui;

import 'models/lat_lng.dart' as l;

class HuaweiMapLocationPicker extends StatefulWidget {
  const HuaweiMapLocationPicker(
  ...
  final Function(HuaweiMapController controller)? onMapCreated;
...
class _HuaweiMapLocationPickerState extends State<HuaweiMapLocationPicker> {
  Completer<HuaweiMapController> completer = Completer();
        ...
        if (_markersSelectedMap.containsKey(key) == true) {
          Marker currentMarker = Marker(
              ...
              onClick: () {
                ...
                if (infoWindow != null) {
                  _infoWindowController.addInfoWindow?.call(infoWindow, l.LatLng(key.lat, key.lng));
                }
  ...
  @override
  void didUpdateWidget(covariant HuaweiMapLocationPicker oldWidget) {
    WidgetsBinding.instance.addPostFrameCallback((timeStamp) async {
      //_updateCurrentPosition();
      ...
      children: [
        HuaweiMap(
          ...
          onMapCreated: (controller) {
            ...
            _infoWindowController.mapController = MapController(ServiceType.HMS)..setController(controller);
  ...
  Future zoomToFit() async {
    ...
    if (widget.myLocation != null && selectedMarker != null) {
      LatLngBounds bounds = LatLngBounds(
        southwest: LatLng(
          widget.myLocation!.lat < selectedMarker!.position.lat
              ? widget.myLocation!.lat
              : selectedMarker!.position.lat,
          widget.myLocation!.lng < selectedMarker!.position.lng
  ...
  LatLngBounds _createBounds(List<LatLng> positions) {
    final southwestLat = positions.map((p) => p.lat).reduce(
            (value, element) => value < element ? value : element); // smallest
    final southwestLon = positions
        .map((p) => p.lng)
        .reduce((value, element) => value < element ? value : element);
    final northeastLat = positions.map((p) => p.lat).reduce(
