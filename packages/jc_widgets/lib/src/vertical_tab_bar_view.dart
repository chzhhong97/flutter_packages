...
class VerticalTabBarView extends StatefulWidget {
  final int itemCount;
  final Widget Function(BuildContext context, int index, bool currentTab)?
      tabBarBuilder;
...
class _VerticalTabBarViewState extends State<VerticalTabBarView>
  ...
  Duration get tabAnimateDuration => Duration(
        microseconds: widget.tabAnimatedDuration.inMicroseconds +
            (widget.tabAnimatedDuration.inMicroseconds /
                    10 *
  ...
  void _setupTabController() {
    ...
    SchedulerBinding.instance.addPostFrameCallback((timeStamp) {
      currentTabContext.value = globalList.isNotEmpty
          ? globalList[currentIndex].currentContext
          : null;
    });
  }
                ...
                child: Stack(
                  ...
                  children: [
                    ValueListenableBuilder(
                      ...
                      builder: (BuildContext context, BuildContext? value,
                          Widget? child) {
                        if (value?.findRenderObject() != null &&
                    ...
                    Column(
                      ...
                      children: List.generate(widget.itemCount, (index) {
                        Widget tab = widget.tabBarBuilder
                                ?.call(context, index, index == currentIndex) ??
                            Tab(
                              text: 'Tab $index',
                            );
        ...
        Expanded(
          ...
          child: PageView.builder(
            ...
            itemBuilder: (context, index) {
              if (widget.tabBarViewBuilder != null)
                return widget.tabBarViewBuilder!(context, index);
          ...
          if (tab.text != null)
            Flexible(
              child: Text(
                ...
                softWrap: true,
                style:
                    currentTab ? widget.textStyle : widget.unselectedTextStyle,
              ),
            )
        ],
      ),
  ...
  void _scrollToItem(GlobalKey key) async {
    if (key.currentContext != null) {
      Scrollable.ensureVisible(key.currentContext!,
          alignment: .5, duration: const Duration(milliseconds: 300));
